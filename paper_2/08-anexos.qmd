...
# Combinar
plot_comp_protestas + plot_comp_estatal +
  plot_annotation(
    tag_levels = 'A',
...
```{r}
#| label: tbl-2019-componentes-educ
#| tbl-cap: "Justificación de violencia en 2019 por educación y participación: todas las variables"
#| echo: false
#| warning: false

elsoc_final_2 %>%
  filter(year == 2019, !is.na(educ_cat_unordered)) %>%
  group_by(educ_cat_unordered, 
           Participacion = factor(protesta_dummy, labels = c("No", "Sí"))) %>%
  summarise(
    N = n(),
    Trab = round(mean(violencia_trabajadores, na.rm = TRUE), 2),
    Estud = round(mean(violencia_estudiantes, na.rm = TRUE), 2),
    Inmob = round(mean(violencia_inmobiliario, na.rm = TRUE), 2),
    Trans = round(mean(violencia_transporte, na.rm = TRUE), 2),
    Local = round(mean(violencia_locales, na.rm = TRUE), 2),
    CMar = round(mean(violencia_carabineros_marchas, na.rm = TRUE), 2),
    CTom = round(mean(violencia_carabineros_tomas, na.rm = TRUE), 2),
    .groups = "drop"
  ) %>%
  knitr::kable(
    col.names = c("Educación", "Part.", "N", "Trab.", "Estud.", "Inmob.", "Trans.", "Local", "C.Mar", "C.Tom"),
  ...
  add_header_above(c(" " = 2, " " = 1, "Viol. Protestas" = 5, "Viol. Estatal" = 2)) %>%
  footnote(
    general = "2019 únicamente. Escala 1-5. Trab=Trabajadores, Estud=Estudiantes, Inmob=Inmobiliario, Trans=Transporte, Local=Locales, C.Mar=Carab. Marchas, C.Tom=Carab. Tomas",
    general_title = "Nota:",
...
```{r}
#| label: tbl-2019-componentes-clase
#| tbl-cap: "Justificación de violencia en 2019 por clase social y participación: todas las variables"
#| echo: false
#| warning: false

elsoc_final_2 %>%
  filter(year == 2019, !is.na(egp3)) %>%
  group_by(egp3, 
           Participacion = factor(protesta_dummy, labels = c("No", "Sí"))) %>%
  summarise(
    N = n(),
  ...
  knitr::kable(
    col.names = c("Clase Social", "Part.", "N", "Trab.", "Estud.", "Inmob.", "Trans.", "Local", "C.Mar", "C.Tom"),
    align = c("l", "c", "c", "c", "c", "c", "c", "c", "c", "c")
  ) %>%
  kable_styling(
    bootstrap_options = c("striped", "hover", "condensed"),
  ...
  add_header_above(c(" " = 2, " " = 1, "Viol. Protestas" = 5, "Viol. Estatal" = 2)) %>%
  footnote(
    general = "2019 únicamente. Escala 1-5. Ver nota anterior para abreviaturas.",
    general_title = "Nota:"
  )
```
...
diag_h1 <- get_diagnostics(mod_educ_protesta, "H1: Educ × Protestas")
diag_h2 <- get_diagnostics(mod_educ_estatal, "H2: Educ × Estatal")
diag_h3a <- get_diagnostics(mod_clase_protesta, "H3a: Clase × Protestas")
diag_h3b <- get_diagnostics(mod_clase_estatal, "H3b: Clase × Estatal")
...
mod_h1_2019 <- glmmTMB(
  justif_violencia_protesta_consistente ~ educ_cat_unordered * protesta_dummy +
    edad + mujer + ideologia_std + (1 | idencuesta),
  data = elsoc_2019, family = gaussian()
)

mod_h3a_2019 <- glmmTMB(
  justif_violencia_protesta_consistente ~ egp3 * protesta_dummy +
    edad + mujer + ideologia_std + (1 | idencuesta),
  data = elsoc_2019, family = gaussian()
)
...
extract_2019 <- function(model_full, model_2019, model_name) {
  ...
  coefs_2019 <- tidy(model_2019, effects = "fixed", conf.int = TRUE) %>%
    filter(grepl("protesta_dummy|:", term)) %>%
    select(term, estimate, std.error, conf.low, conf.high) %>%
...
# Primero separamos los datos por participación
...
ggplot(slopes_data, aes(x = educ_ordered, y = slope)) +
  ...
  labs(
    ...
    caption = "Barras muestran diferencias observadas; líneas verticales: IC 95%. Línea punteada en 0: sin efecto de participación.\nElaboración propia con ELSOC 2016-2023."
  ) +
  theme_minimal(base_size = 10) +
  theme(
...
ggplot(slopes_nopart_data, aes(x = educ_ordered, y = slope_educ)) +
  geom_hline(yintercept = 0, linetype = "dashed", color = "gray50", linewidth = 0.8) +
  ...
  labs(
    ...
    caption = "Barras muestran diferencias respecto a educación media; líneas verticales: IC 95%. Línea punteada en 0: sin diferencia.\nElaboración propia con ELSOC 2016-2023."
  ) +
  theme_minimal(base_size = 10) +
  theme(
    axis.text.x = element_text(angle = 35, hjust = 1, size = 9),
